...
def make_wrapper(work: Path) -> tuple[Path, str]:
    ...
    w.write_text(
        ...
        "# Bridge native (parent) <-> child stdio via subprocess.PIPE so that\n"
        "# cwrsync (cygwin) gets pipes it can sniff as its own. Without the\n"
        "# bridge, cwrsync inherits the parent native handles directly and\n"
        "# deadlocks instead of speaking protocol.\n"
        "p = subprocess.Popen(args, stdin=subprocess.PIPE, stdout=subprocess.PIPE,\n"
        "                     stderr=sys.stderr, env=env, bufsize=0)\n"
        "def pump_fd_to_pipe(src_fd, dst, label):\n"
        "    try:\n"
        "        while True:\n"
        "            buf = os.read(src_fd, 65536)\n"
        "            if not buf:\n"
        "                break\n"
        "            dst.write(buf)\n"
        "            try: dst.flush()\n"
        ...
        "        with open(LOG, 'a', encoding='utf-8') as f:\n"
        "            f.write(f'  pump {label} err: {e!r}\\n')\n"
        "    finally:\n"
        "        try: dst.close()\n"
        "        except Exception: pass\n"
        "def pump_pipe_to_fd(src, dst_fd, label):\n"
        "    try:\n"
        "        while True:\n"
        "            buf = src.read(65536)\n"
        "            if not buf:\n"
        ...
        "            n = 0\n"
        "            while n < len(buf):\n"
        "                n += os.write(dst_fd, buf[n:])\n"
        "    except Exception as e:\n"
        "        with open(LOG, 'a', encoding='utf-8') as f:\n"
        "            f.write(f'  pump {label} err: {e!r}\\n')\n"
        "t1 = threading.Thread(target=pump_fd_to_pipe, args=(0, p.stdin, 'in->child'), daemon=True)\n"
        "t2 = threading.Thread(target=pump_pipe_to_fd, args=(p.stdout, 1, 'child->out'), daemon=True)\n"
        "t1.start(); t2.start()\n"
        "rc = p.wait()\n"
        "t2.join(timeout=5)\n"
        "with open(LOG, 'a', encoding='utf-8') as f:\n"
